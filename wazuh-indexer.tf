...
certbot certonly --dns-route53 -d 127cyber.com -d wazuh.127cyber.com -d listen.127cyber.com --non-interactive --agree-tos -m thomas_hamer@outlook.com
...
awk '/BEGIN CERTIFICATE/,/END CERTIFICATE/' /etc/letsencrypt/live/127cyber.com/fullchain.pem > /etc/wazuh-indexer/certs/root-ca.pem
...
awk '/BEGIN CERTIFICATE/,/END CERTIFICATE/' /etc/letsencrypt/live/127cyber.com/fullchain.pem > /etc/wazuh-dashboard/certs/root-ca.pem
...
JAVA_HOME=/usr/share/wazuh-indexer/jdk runuser wazuh-indexer --shell=/bin/bash --command="cd /etc/wazuh-indexer/opensearch-security && /usr/share/wazuh-indexer/plugins/opensearch-security/tools/securityadmin.sh -cacert /etc/wazuh-indexer/certs/root-ca.pem -cert /etc/wazuh-indexer/certs/indexer.pem -key /etc/wazuh-indexer/certs/indexer-key.pem -h 127.0.0.1 -p 9200 -icl -nhnv"
  TEOF
}
